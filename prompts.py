...
def build_work_planning_prompt(objective: str, strategy: str, files_to_edit: list, files_to_create: list) -> str:
    return (
        ...
        f"2. For EXISTING files, create a task for EACH specific method or logic block you need to change. You MUST specify the `files_to_edit` parameter.\n"
        f"3. For NEW files, Task 1 must be creating the skeleton (imports, empty classes/methods), followed by tasks for each method. You MUST specify the `files_to_edit` parameter.\n"
...
def build_planning_prompt(objective: str, specs_summary: str) -> str:
    return (
        ...
        f"7. You MUST specify the `files_to_edit` parameter for EVERY task (e.g. files_to_edit='database.py').\n\n"
        f"EXAMPLE of Micro-Tasks for 'database.py':\n"
        f"  add_project_task(description='Create skeleton for database.py', files_to_edit='database.py')\n"
        f"  add_project_task(description='Implement DB.__init__ method in database.py', files_to_edit='database.py')\n"
        f"  add_project_task(description='Implement DB.save method in database.py', files_to_edit='database.py')\n\n"
